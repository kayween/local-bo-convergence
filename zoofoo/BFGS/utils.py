...
def compute_stats(X_train, y_train, X_test, y_test, opfun, accfun, ghost_batch=128):
    ...
    for smpl in np.array_split(
        np.random.permutation(range(X_test.shape[0])),
        int(X_test.shape[0] / ghost_batch),
    ):
        ...
        if torch.cuda.is_available():
            test_loss += F.cross_entropy(testops, test_tgts).cpu().item() * (
                len(smpl) / X_test.shape[0]
            )
        else:
            test_loss += F.cross_entropy(testops, test_tgts).item() * (
                len(smpl) / X_test.shape[0]
            )
    ...
    for smpl in np.array_split(
        np.random.permutation(range(X_train.shape[0])),
        int(X_test.shape[0] / ghost_batch),
    ):
        ...
        if torch.cuda.is_available():
            train_loss += F.cross_entropy(trainops, train_tgts).cpu().item() * (
                len(smpl) / X_train.shape[0]
            )
        else:
            train_loss += F.cross_entropy(trainops, train_tgts).item() * (
                len(smpl) / X_train.shape[0]
            )
