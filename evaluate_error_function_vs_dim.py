...
def evaluate_error_functions(batch, d, kernel, noise, device):
    ...
    for i in range(5):
        ...
        print("batch {:3d}, dim {:3d}, value {:.6f}".format(batch, dim, value))
...
if __name__ == "__main__":
    ...
    exponent = np.linspace(0., 2., num=10)
    lst_dim = np.rint(10 ** exponent).astype(int)
    lst_error = []

    for dim in lst_dim:
        error = evaluate_error_functions(
            batch=args.n,
            d=dim, kernel=args.kernel, noise=args.stddev ** 2,
            device=device,
        )
